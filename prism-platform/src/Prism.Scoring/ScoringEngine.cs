...
/// so Used=false is real evidence of an idle Copilot seat — the clearest, priciest reclaim.</summary>
public sealed record CopilotSignal(bool Used, int? DaysSinceLast, bool HasReport);
...
public sealed record AssignmentVerdict(
    ...
    int? InactiveDays, decimal? MonthlySavings);
...
public static class ScoringEngine
{
    public static AssignmentVerdict ScoreAssignment(
        ...
        decimal? unitCost, string currency, ScoringOptions o, DateTime nowUtc)
    {
        ...
        // Service / shared-account suspicion => cap at REVIEW (classic false positives).
        bool serviceSuspect = (dInteractive is null || dInteractive >= o.ReviewDays)
                              && dNonInter is { } ni && ni < o.WarnDays;     // app/non-interactive only
        bool nameSuspect = MatchesAny(r.UserPrincipalName, o.ServiceAccountPatterns)
                          || MatchesAny(r.DisplayName, o.ServiceAccountPatterns);
        if (serviceSuspect) reasons.Add(Reason.ServiceAccountSuspected);
        if (nameSuspect)    reasons.Add(Reason.PossibleSharedOrService);
        bool capReview = serviceSuspect || nameSuspect;
        ...
        if (copilot is { Used: false, HasReport: true })
        {
            ...
            AddHighValue(r, o, reasons);
            return (o.EnableCopilotReclaim && !capReview)
        ...
        if (teams is { CallCount: 0, HasReport: true })
        {
            reasons.Add(Reason.PhoneNoCalls);
            AddHighValue(r, o, reasons);
            return (o.EnableTeamsPhoneReclaim && !capReview)
                ? new(Decision.Reclaim, 90, Conf.High,   reasons, eff, unitCost)
                : new(Decision.Review,  80, Conf.Medium, reasons, eff, unitCost);
        }
        ...
        // ---- banding ----
        string decision =
            (score >= o.ReclaimScore && conf == Conf.High && !capReview) ? Decision.Reclaim
