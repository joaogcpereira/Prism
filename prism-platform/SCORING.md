...
`POSSIBLE_SHARED_OR_SERVICE`, `APP_IN_USE`, `APP_UNUSED`, `NEVER_ACTIVE`,
...
- **Shared-mailbox / service-account detection is heuristic** (name patterns + sign-in
  shape). Reliable detection needs mailbox `RecipientTypeDetails`, which isn't ingested yet —
  a worthwhile next connector enhancement.
